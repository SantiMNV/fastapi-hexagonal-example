...
class TestSQLAlchemyPostRepository:
    def test_add_get_and_list(self, db_session: Session) -> None:
        ...
        loaded = posts.get_by_id(post.id)
        ...
        listed = posts.list_by_user_id(user.id)
        assert len(listed) == 1
    ...
    def test_get_by_id_returns_none_when_missing(self, db_session: Session) -> None:
        ...
        assert posts.get_by_id("missing") is None
    ...
    def test_delete_is_noop_when_row_missing(self, db_session: Session) -> None:
        ...
        assert posts.get_by_id("nope") is None
